...
class RpController extends Controller
{
    public function index()
    {
        ...
        $query = Plan::select('group_id', 'subject_id');        
        if($subject) {
        ...
        $programs = $query->groupBy('group_id', 'subject_id')->paginate(30);
    ...
    public function view($groupId, $subjectId)
    {
        ...
        $group['codes'] = $group->codes; //иначе не работает
    ...
    public function store(Request $request, $groupId, $subjectId)
    {
        ...
        foreach ($parts as $key => $p) {
            ...
            foreach ($p['lessons'] as $k => $l) {
                ...
                if($l['practice'] && @$parallel[$paralOrder]->plan->semestr == @$main[$order]->plan->semestr) {
  ...
  foreach ($parts as $key => $part) { 
    ...
    foreach($part['lessons'] as $l) {   
      $section->addText("Тема ".++$partNum.".".++$lesNum.". ".$l->topic,
          null,
          array(
