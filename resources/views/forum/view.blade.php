					...
					<div class="card message" :id="message.id">
						<div class="card-body p-2 rounded" style="overflow: hidden;" :class="{'bg-secondary text-white': message.user_id==user.id}">
							...
							<small class="text-muted mx-2 mb-1 reply-link" @click="reply = message">
								<img src="/public/img/icons/reply.svg" height="15" >
							</small>
							<small v-if="message.user_id == user.id" class="text-muted mx-2 mb-1 reply-link" @click="toDelete = message" data-toggle="modal" data-target="#delete">
								<img src="/public/img/icons/delete.svg" height="15">
							</small>
						</div>
					</div>
					<div class="modal fade" id="delete" v-show="toDelete">
						<div class="modal-dialog">
							<div class="modal-content">
									...
									<div class="modal-footer">
										<button type="button" class="btn btn-secondary" data-dismiss="modal" @click="deleteMessage">Да</button>
										<button type="button" class="btn btn-light" data-dismiss="modal" @click="toDelete=null">Нет</button>
									</div>
								</form>
							</div>
						</div>
			...
			const app = new Vue({
				...
				methods: {
					...
					deleteMessage: function() {
						var index = this.messages.findIndex(v => v.id == this.toDelete.id)
						axios.get('/api/forum/' + this.toDelete.id + '/delete?api_token=' + this.user.api_token)
						.then(() => {
